...
// SystemVerilog created from k0_ZTS9VectorAdd_function_cra_slave
...
module k0_ZTS9VectorAdd_function_cra_slave (
    ...
    // printf_bit(BITSELECT,183)
    assign printf_bit_b = status_NO_SHIFT_REG_q[3:3];

    // printf_bit_mux(MUX,184)
    ...
    // readdata_valid_stage1_reg(REG,191)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // finish_counter_NO_SHIFT_REG(REG,87)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // printf_reset_bit(BITSELECT,186)
    ...
    // cra_output_was_finish_counter_address_stage2_reg(REG,82)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdata_reg(REG,79)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // readdata_valid_stage2_reg(REG,192)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // cra_output_readdatavalid_reg(REG,80)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0_buffered(REG,104)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_0(REG,103)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0_buffered(REG,98)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_0(REG,97)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_0_bit_join(BITJOIN,94)
    ...
    // global_offset_reg_upper_1_buffered(REG,106)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_1(REG,105)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1_buffered(REG,100)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_1(REG,99)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_1_bit_join(BITJOIN,95)
    ...
    // global_offset_reg_upper_2_buffered(REG,108)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_upper_2(REG,107)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2_buffered(REG,102)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_reg_lower_2(REG,101)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_offset_2_bit_join(BITJOIN,96)
    ...
    // global_size_reg_upper_0(REG,118)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_reg_lower_0(REG,112)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_0_bit_join(BITJOIN,109)
    ...
    // global_size_reg_lower_1(REG,114)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_1_bit_join(BITJOIN,110)
    ...
    // global_size_reg_lower_2(REG,116)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // global_size_2_bit_join(BITJOIN,111)
    ...
    // dupName_10_arg_bit_join_x(BITJOIN,295)
    ...
    // dupName_9_arg_bit_join_x(BITJOIN,287)
    ...
    // dupName_8_arg_bit_join_x(BITJOIN,279)
    ...
    // dupName_7_arg_bit_join_x(BITJOIN,271)
    ...
    // dupName_6_arg_bit_join_x(BITJOIN,263)
    ...
    // dupName_5_arg_bit_join_x(BITJOIN,255)
    ...
    // dupName_4_arg_bit_join_x(BITJOIN,248)
    ...
    // dupName_3_arg_bit_join_x(BITJOIN,243)
    ...
    // dupName_2_arg_bit_join_x(BITJOIN,238)
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,233)
    ...
    // dupName_0_arg_bit_join_x(BITJOIN,226)
    ...
    // kernel_arg_bit_join(BITJOIN,137)
    ...
    // local_size_reg_2_buffered(REG,144)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // not_started(LOGICAL,154)
    assign not_started_q = ~ (started_NO_SHIFT_REG_q);

    // start_is_or_going_high(LOGICAL,208)
    ...
    // workgroup_size_NO_SHIFT_REG(REG,224)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
