 ...
 */
function searchJsonArrayByName(jsonArray, name, jsonFunc){
  var res = null;
  for (let i = 0; i < jsonArray.length; i++){
    if (jsonArray[i]['name'] === name) { res = jsonArray[i]; break; }
    // The name can be a mangled name if it's starts with Kernel in SYCL
    if (name.indexOf("Kernel: ") >= 0 && jsonFunc !== null &&
       (jsonHasChildrenArray(jsonArray[i]) && jsonHasChildrenArray(jsonFunc)) &&
       jsonArray[i]['children'].length > 0) {
      // Try to find first matching loop block as those names will match
      let userFriendlyName = jsonArray[i]['children'][0]['name'];
      if (searchJsonArrayByName(jsonFunc['children'], userFriendlyName)) {
        res = jsonArray[i]; 
        break;
      }
    }
    else if (jsonHasChildrenArray(jsonArray[i])) res = searchJsonArrayByName(jsonArray[i]['children'], name);
    ...
    if (loop_attrJSON.hasOwnProperty('nodes')) {
      if (vTreeNode.type === 'system') {
        ...
        loop_attrJSON['nodes'].forEach(function(vFuncObj) {
          ...
          vOldLoopJson = searchJsonArrayByName(loopsJSON['children'], vFuncType+vFuncName, vFuncObj);
          if (!vOldLoopJson) {
